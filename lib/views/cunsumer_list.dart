...
class ConsumerList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child:
            GetX<ConsumerController>(builder: (controller) {
              return consumerController.isLoading.value ? Center(child:  SizedBox(
                  height: 100,
                  width: 100,
                  child:  CircularProgressIndicator(strokeWidth: 10,color: Colors.teal))):Column(
                children: [
                  Expanded(
                    child: ListView.builder(
                        itemCount: controller.consumerResponse!.consumers!.length,
